...
/obj/machinery/airalarm
	...
	var/list/datum/tlv/tlv_collection = list()

	/// Tlv data to set in init, most gasses are built there but won't override these if set
	var/list/tlv_data = list(
		"pressure" = /datum/tlv/pressure,
		"temperature" = /datum/tlv/temperature,
	)
...
/obj/machinery/airalarm/vox
	tlv_data = list(
		"pressure" = /datum/tlv/pressure,
		"temperature" = /datum/tlv/temperature,
		/datum/gas/oxygen = /datum/tlv/vox_oxygen,
		/datum/gas/nitrogen = /datum/tlv/vox_nitrogen,
		/datum/gas/nitrous_oxide = /datum/tlv/no_checks,
		/datum/gas/carbon_dioxide = /datum/tlv/no_checks,
	)

MAPPING_DIRECTIONAL_HELPERS(/obj/machinery/airalarm/vox, 27)
...
/obj/machinery/airalarm/proc/setup_tlv()
	for(var/key in tlv_data)
		var/datum/tlv/tlv_type = tlv_data[key]
		tvl_set_key(key, tlv_type, replace = FALSE)

	var/list/meta_info = GLOB.meta_gas_info // shorthand
	for(var/gas_path in meta_info)
		if(ispath(gas_path, /datum/gas/oxygen))
			tvl_set_key(gas_path, /datum/tlv/oxygen, replace = FALSE)
		else if(ispath(gas_path, /datum/gas/carbon_dioxide))
			tvl_set_key(gas_path, /datum/tlv/carbon_dioxide, replace = FALSE)
		else if(meta_info[gas_path][META_GAS_DANGER])
			tvl_set_key(gas_path, /datum/tlv/dangerous, replace = FALSE)
		else
			tvl_set_key(gas_path, /datum/tlv/no_checks, replace = FALSE)

/obj/machinery/airalarm/proc/tvl_set_key(collection_key, datum/tlv/tlv_type, replace = TRUE)
	var/datum/tlv/existing = tlv_collection[collection_key]
	if(existing)
		if(!replace || ispath(tlv_type, existing.type))
			return
		QDEL_NULL(existing)

	tlv_collection[collection_key] = new tlv_type
