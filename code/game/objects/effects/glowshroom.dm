/obj/structure/glowshroom
	...
	/// Endurance of the orginal seeds
...
/obj/structure/glowshroom/single/no_decay
...
/obj/structure/glowshroom/Initialize(mapload, obj/item/seeds/newseed, icon_variant)
	...
	// Only process glowshrooms that WILL process
	if(spread_into_adjacent_chance && endurance_decay_rate)
...
/obj/structure/glowshroom/proc/spread(seconds_per_tick)
	if(!spread_into_adjacent_chance)
		return
	...
	for(var/i in 1 to myseed.yield)
		...
		for(var/iterator in 1 to min(length(possible_locs), 3))
			...
			if(SPT_PROB(spread_into_adjacent_chance, seconds_per_tick) && !(locate(/obj/structure/glowshroom) in possibleLoc))
				new_loc = possibleLoc
				break
